...
(p.hat1 <- resp.1 / (resp.1 + no.resp.1)) # proportion of success
(p.hat2 <- resp.2 / (resp.2 + no.resp.2))

(attributable.risk <- p.hat1 - p.hat2) # or risk difference
(SE <- sqrt(p.hat1 * (1 - p.hat1) / (resp.1 + no.resp.1) + p.hat2 * (1 - p.hat2) / (resp.2 + no.resp.2)))
...
(SE <- sqrt(1/resp.1 - 1/(resp.1+no.resp.1) + 1/resp.2 - 1/(resp.2+no.resp.2)))
(log.rr.ci <- log.risk.ratio + c(-2,2) * SE) # CI95 for log.risk.ratio
(rr.ci <- exp(log.rr.ci)) # CI95 for risk.ratio; is significant if both are > 1 or < 1
...
(or.ci <- exp(log.or.ci)) # CI95 for or.ci; is significant if both are > 1 or < 1
...
# Estimates of all ci95 in % - conclusion
